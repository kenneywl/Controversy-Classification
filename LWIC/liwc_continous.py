...
# su = pd.read_excel("Summaries.xlsx",nrows=1000)['somewhat controversial']
# su.index = art.index
# new = []
# for i in art.index:
# 	new += [art.loc[i,"Response"]/(1+su.loc[i])]
# art['Response'] = new

fa = pd.read_excel("Factors.xlsx").iloc[:,2:]
...
def rank_cols(results,x,thresh):
	ranks = results.pvalues.rank().astype(int).apply(lambda x: x-1).tolist()
	rank_in = []
	...
	x = x.iloc[:,pr_rank]
	ncols = sum(np.array(results.pvalues < thresh,dtype=int))
	x = x.iloc[:,:ncols]
	if "Pos_Score" not in x.columns:
		x = x.assign(Pos_Score=art.loc[:,"Pos_Score"])
	return(x)

x = rank_cols(result,x,.2)
result2 = sm.OLS(y,x).fit()
x = rank_cols(result2,x,1)
...
# res = [average_meterics(i) for i in th]
# plt.scatter(th,res)
# plt.show()